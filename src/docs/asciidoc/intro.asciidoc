...
GeoScript Groovy also comes packaged as an uber jar which contains all dependencies.  Download geoscript-groovy-app-1.22.0-SNAPSHOT.jar
from github.

[source,bash]
.Run uber jar from the command line.
----
java -jar geoscript-groovy-app-1.22.0-SNAPSHOT.jar
----
...
----
dependencies {
    compile("org.geoscript:geoscript-groovy:1.22.0-SNAPSHOT")
}
----